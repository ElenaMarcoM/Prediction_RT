...
def data_from_pkl(pkl_path):
    ...
    :return:
    """
    fingerprints = pd.read_pickle(pkl_path)
...
def data_from_csv(csv_path):
    """
    Reads data from csv file and separates the fingerprints from the rt for the model.

    :param csv_path: path to csv file that contains rt data
    :return:
    """
...
# --- CREATING MODEL ---
def create_model(pkl_path):
    """
    Creates, trains and tests the model given the rt data file.
    :param pkl_path: path to file with fingerprints and rt
    """
    X, y = data_from_pkl(pkl_path)
